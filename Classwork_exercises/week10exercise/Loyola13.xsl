...
<xsl:template match="campus">
  <tr>
    ...
    <xsl:apply-templates select="building/number-of-floors?????"/>
...
<xsl:templates match="building/number-of-floors"?????/>
  <td>
   <xsl:value-of select="sum(./number-of-floors) div count(./Loyola/campus/building)"/>
  </td>
</xsl:template>
